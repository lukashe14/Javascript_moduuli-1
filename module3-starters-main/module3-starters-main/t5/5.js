...
// add your code here
const kohta = document.querySelector('section');

picArray.forEach(itemi => {
  const artikkeli = document.createElement('article');
  artikkeli.classList.add('card');

  const h2 = document.createElement('h2');
  h2.textContent = itemi.title;

  const figure = document.createElement('figure');

  const img = document.createElement('img');
  img.src = itemi.image.medium;
  img.alt = itemi.title;
  figure.appendChild(img);

  const caption = document.createElement('figcaption');
  caption.textContent = itemi.caption;
  figure.appendChild(caption);

  artikkeli.appendChild(figure);

  const p = document.createElement('p');
  p.textContent = itemi.description;
  artikkeli.appendChild(p);

  kohta.appendChild(artikkeli);
});
